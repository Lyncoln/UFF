...
$\hat{Y_i}$: É o valor estimado do percentual de pacientes infectados para i i-ésimo estabelecimento de saúde;   
...
\begin{center} 
onde,
$$\hat{Var}(\beta_1) = \frac{\hat{\sigma}^2}{\sum\limits_{i=1}^{n}(X_i-\bar{X})^2};$$
desta forma,
$$t_{obs} = 5.993;$$
\end{center}
...
Como obteve-se um $t_{obs} = 5.993$, então $t_{obs} \in RC$ (verifique a \hyperref[fig:rcb1]{\underline{figura \ref*{fig:rcb1}}}), desta forma, rejeitamos a hipótese nula. Conclui-se. com base no teste $T$ que existe uma relação estatisticamente  significante entre tempo médio de internação ($X$) e percentual de pacientes infectados ($Y$).
...
Como obteve-se um $f_{obs} = 35.922$, então $f_{obs} \in RC$ (verifique a \hyperref[fig:rcf]{\underline{figura \ref*{fig:rcf}}}), desta forma, rejeitamos a hipótese nula. Conclui-se. com base no teste $F$ que existe uma relação estatisticamente  significante entre tempo médio de internação ($X$) e percentual de pacientes infectados ($Y$).
...
\begin{itemize}
  ...
  \item Normalidade:                       \\
   Pela \hyperref[fig:qqplot]{\underline{figura \ref*{fig:qqplot}}}, pode-se notar que os quantis  dos resíduos studentizados se aproximam dos quantis teóricos de uma distribuição normal com média 0 e variância $\sigma^2$, o que é um bom indicativo de normalidade dos erros.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccccc}
   ...
   Estimadores & Estimativa & Erro padrão & Valor da estatística de teste T &  P-valor                                                       \\
    ...
    $\hat{\beta_1}$  & 0.47580  & 0.07939 & 5.993  & 3.42e-08 \\
    \hline
  \end{tabular}
  \label{tab:summary}
\end{table}
